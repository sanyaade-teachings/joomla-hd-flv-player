...
");
	...
	$updateDid = AddColumnIfNotExists($errorMsg, "#__hdflvplayersettings", "ads_type", "int(11) NOT NULL", "googleanalyticsID");
	if (!$updateDid) {
		$msgSQL .= "error adding 'ads_type' column to 'hdflvplayersettings' table <br />";
	}

	$updateDid = AddColumnIfNotExists($errorMsg, "#__hdflvplayersettings", "IMAAds_path", "varchar(255) NOT NULL", "ads_type");
	if (!$updateDid) {
		$msgSQL .= "error adding 'IMAAds_path' column to 'hdflvplayersettings' table <br />";
	}
	...
	if ($db->getErrorNum()) {
		...
		if (!check_column('hdflvplayerlanguage', 'youtube_video_notallow', '', 'VARCHAR(300) NOT NULL')) {
			$msgSQL .= "error adding 'youtube_video_notallow' column to 'hdflvplayerlanguage' table <br />";
		}
		if (!check_column('hdflvplayerlanguage', 'not_permission', '', 'VARCHAR(300) NOT NULL')) {
			$msgSQL .= "error adding 'not_permission' column to 'hdflvplayerlanguage' table <br />";
		}
		if (!check_column('hdflvplayerlanguage', 'tumblr', '', 'VARCHAR(255) NOT NULL')) {
			$msgSQL .= "error adding 'tumblr' column to 'hdflvplayerlanguage' table <br />";
		}
		if (!check_column('hdflvplayerlanguage', 'googleplus', '', 'VARCHAR(255) NOT NULL')) {
			$msgSQL .= "error adding 'googleplus' column to 'hdflvplayerlanguage' table <br />";
		}
		if (!check_column('hdflvplayerlanguage', 'youtube_video_url_incorrect', '', 'VARCHAR(300) NOT NULL')) {
			$msgSQL .= "error adding 'youtube_video_url_incorrect' column to 'hdflvplayerlanguage' table <br />";
		}
		if (!check_column('hdflvplayerlanguage', 'youtube_video_removed', '', 'VARCHAR(300) NOT NULL')) {
			$msgSQL .= "error adding 'youtube_video_removed' column to 'hdflvplayerlanguage' table <br />";
		}
		if (!check_column('hdflvplayerlanguage', 'login', '', 'VARCHAR(50) NOT NULL')) {
			$msgSQL .= "error adding 'login' column to 'hdflvplayerlanguage' table <br />";
		}
