 ...
 */
class hdflvplayerModeleditvideoupload extends JModelLegacy {
	...
	function addvideouploadmodel()
	{
		...
		$db->setQuery( $query );
		$rs_playersettings = $db->loadObject();
		
		if(!empty($rs_playersettings))
		{
			// To set max file size in php.ini
			ini_set('upload_max_filesize', $rs_playersettings->uploadmaxsize."M"); // to assign value to the php.ini file
			...
			$upload_maxsize = $rs_playersettings->uploadmaxsize;
		}
		...
		$query  = 'SELECT id,adsname FROM #__hdflvplayerads
             			WHERE typeofadd != \'mid\' and published=1 ORDER BY adsname ASC';
		$db->setQuery( $query);
...
//                session_regenerate_id();
		...
		$rs_playlistname = $db->loadObjectList();
				...
				//Generates file name with target path based on session id 
