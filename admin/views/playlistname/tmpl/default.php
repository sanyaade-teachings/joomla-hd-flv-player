...
<form action="index.php?option=com_hdflvplayer&task=playlistname" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data">
    ...
    <input type="text" name="search" id="search" value="<?php if (isset($rs_showplaylistname['lists']['search']))
    echo $rs_showplaylistname['lists']['search']; ?>"  onchange="document.adminForm.submit();" />
    ...
    <table class="adminlist">
        <thead>
            <tr>
                ...
                <th width="6%">
<?php echo JHTML::_('grid.sort', 'COM_HDFLVPLAYER_VIEW_PLAYLISTNAME_TMPL_NAME', 'name', @$rs_showplaylistname['lists']['order_Dir'], @$rs_showplaylistname['lists']['order']); ?>

                </th>

                <th width="1%"><?php echo JText::_('COM_HDFLVPLAYER_PUBLISHED');?></th>
            </tr>
        </thead>
        <?php
        $k = 0;
...
//echo $rs_showplaylistname[0]->name;
    ...
    <?php
        $rsplay->id = isset($rsplay->id) ? $rsplay->id : '';
