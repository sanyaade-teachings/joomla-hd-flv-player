...
if (!empty($rs_editsettings)) {
    ...
    <?php if ($rs_editsettings->googleana_visible == '1') { ?>
googleana_vis(1);
  <?php  }else{ ?>
     googleana_vis(0);
   <?php  }
if ($rs_editsettings->ads_type == '1') { ?>
adtype(1);
  <?php  }else{ ?>
     adtype(0);
   <?php  }
...
function adtype(postvalue)
{
    if(postvalue == 0)
    	{
        document.getElementById("IMAAd_path").style.display = 'none';
    	}
    if(postvalue == 1)
    	{
        document.getElementById("IMAAd_path").style.display = '';
    	}
}
			...
			<table class="adminlist">
				...
				<tbody>
					...
					<tr>
						...
						<td>
						<?php
						$skinoptions[] = JHTML::_('select.option','skin_black.swf','Black');
						$skinoptions[] = JHTML::_('select.option','skin_fancyblack.swf','Fancy Black');
						$skinoptions[] = JHTML::_('select.option','skin_fresh_blue.swf','Fresh Blue');
						$skinoptions[] = JHTML::_('select.option','skin_fresh_yellow.swf','Fresh Yellow');
						$skinoptions[] = JHTML::_('select.option','skin_fresh_white.swf','Fresh White');
						$skinoptions[] = JHTML::_('select.option','skin_neat_fresh_yellow.swf','Neat Yellow');
						$skinoptions[] = JHTML::_('select.option','skin_Overlay.swf','Overlay');
						$skinoptions[] = JHTML::_('select.option','skin_sleekblack.swf','Sleek Black');
						$skinoptions[] = JHTML::_('select.option','skin_white.swf','White');
						$skinoptions[] = JHTML::_('select.option','skin_youtube.swf','Youtube');
					...
					<tr>
						...
						<td><?php
						$googleana_visiblechkyes = $googleana_visiblechkno='';
						if ($rs_editsettings->googleana_visible == '1') {

							{
								$googleana_visiblechkyes = 'checked="checked" ';
							}

						}
						else if ($rs_editsettings->googleana_visible == '0' || $rs_editsettings->googleana_visible == '') {
							$googleana_visiblechkno = 'checked="checked" ';
						}
						?>
                                                     <fieldset id="jform_type" class="radio inputbox">

	<input type="radio" name="googleana_visible" id="googleana_visible1" value="1" <?php echo $googleana_visiblechkyes; ?> onclick="googleana_vis(1);">
	<label for="googleana_visible1">Enable</label>
	<input type="radio" name="googleana_visible" id="googleana_visible0" value="0" <?php echo $googleana_visiblechkno; ?> onclick="googleana_vis(0);">
	<label for="googleana_visible0">Disable</label>
						     </fieldset>
                                                     </td>
					</tr>
...
<!--					<tr>
						...
						<?php echo JHtml::_( 'select.radiolist', $options, 'ads', '', 'value', 'text', $rs_editsettings->ads); ?>
                                        ...
                                        <tr>
						<td><?php echo JHTML::tooltip('Select Ads type', 'Ads Type','', 'Ads Type');?></td>
						<td><?php
						$adtype_visiblechkyes = $adtype_visiblechkno='';
						if ($rs_editsettings->ads_type == '1') {

							{
								$adtype_visiblechkyes = 'checked="checked" ';
							}

						}
						else if ($rs_editsettings->ads_type == '0' || $rs_editsettings->ads_type == '') {
							$adtype_visiblechkno = 'checked="checked" ';
						}
						?>
                                                     <fieldset id="jform_type" class="radio inputbox">

	<input type="radio" name="ads_type" id="adtype_visible1" value="1" <?php echo $adtype_visiblechkyes; ?> onclick="adtype(1);">
	<label for="adtype_visible1">Enable</label>
	<input type="radio" name="ads_type" id="adtype_visible0" value="0" <?php echo $adtype_visiblechkno; ?> onclick="adtype(0);">
	<label for="adtype_visible0">Disable</label>
						     </fieldset>
                                                     </td>
					</tr>
					...
					<tr id="IMAAd_path">
						<td>
							<div id="show"><?php echo JHTML::tooltip('Enter Intermediate Ad Path', 'Intermediate Ad Path','', 'Intermediate Ad Path');?></div>
						</td>

						<td>
							<div id="show1">
								<input name="IMAAds_path" id="IMAAds_path" maxlength="100" value="<?php echo $rs_editsettings->IMAAds_path; ?>"/>
							</div>
						</td>
					</tr>
				</tbody>
