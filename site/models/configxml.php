...
class hdflvplayerModelconfigxml extends HdflvplayerModel {
    ...
    function configgetrecords() {
        ...
        $db = JFactory::getDBO();
        $query = 'SELECT `ads_type`,`IMAAds_path`,`id`, `published`, `buffer`, `normalscale`, `fullscreenscale`, `autoplay`,
    ...
    function configxml($rs_moduleparams, $settingsrows, $playid, $playid_playlistname, $moduleid, $comppid, $base, $allowmidrollads) {
        ...
        $stagecolor = "0x" . $settingsrows->stagecolor;
         ...
         ($settingsrows->ads_type == 0) ? $IMAAds = "false" : $IMAAds = "true";
        if($IMAAds=="true")
            $IMAAds_path=$settingsrows->IMAAds_path;
...
if (version_compare(JVERSION, '2.5', 'ge')|| version_compare(JVERSION, '3.0', 'ge') || version_compare(JVERSION, '1.6', 'ge') || version_compare(JVERSION, '1.7', 'ge')) {
    ...
    ($videocat['videocat'] == 1) ? $videoid = $videoId['videoid'] : $playid_playlistname = $playlistId['playlistid'];
        ...
        $target_url = $settingsrows->logourl;
        if(empty($target_url)){
